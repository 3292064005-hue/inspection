import type { AuditEntry, HmiGateway, GatewayEventName, GatewayHandler, GatewayStatusListener } from '@/shared/gateway/contracts';
import {
  gatewayValidators,
  parseCountStats,
...
export class SafeGateway implements HmiGateway {
  ...
  configureDemoScenario?(scenario: DemoScenario): Promise<void> {
    return this.inner.configureDemoScenario?.(scenario) ?? Promise.resolve();
  }
