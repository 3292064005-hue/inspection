...
class ReadModelPolicy:
    ...
    query_side_trace_refresh: str = READ_MODEL_QUERY_REFRESH_TRACE_STREAM
...
def normalize_query_side_trace_refresh(raw: str | None) -> str:
    normalized = str(raw or READ_MODEL_QUERY_REFRESH_TRACE_STREAM).strip().lower() or READ_MODEL_QUERY_REFRESH_TRACE_STREAM
...
def load_read_model_policy(config_path: str | Path = DEFAULT_POLICY_PATH) -> ReadModelPolicy:
    ...
    query_side_trace_refresh = normalize_query_side_trace_refresh(os.environ.get('INSPECTION_READ_MODEL_QUERY_REFRESH', section.get('query_side_trace_refresh', READ_MODEL_QUERY_REFRESH_TRACE_STREAM)))
    return ReadModelPolicy(mode=mode, bootstrap_repair_on_empty_db=_as_bool(os.environ.get('INSPECTION_READ_MODEL_BOOTSTRAP_REPAIR_ON_EMPTY_DB', section.get('bootstrap_repair_on_empty_db')), default=True), allow_runtime_repair_on_sync_mismatch=_as_bool(os.environ.get('INSPECTION_READ_MODEL_ALLOW_RUNTIME_REPAIR', section.get('allow_runtime_repair_on_sync_mismatch')), default=False), fallback_legacy_reads=_as_bool(os.environ.get('INSPECTION_READ_MODEL_FALLBACK_LEGACY_READS', section.get('fallback_legacy_reads')), default=False), query_side_trace_refresh=query_side_trace_refresh)
