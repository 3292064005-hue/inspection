...
from .read_model_trace_queries import build_batch_summary, fetch_artifacts_for_trace_ids, fetch_result_ids_for_batch, fetch_trace_bundle, fetch_trace_bundles_for_batch, fetch_trace_id_for_result, fetch_trace_ids
...
class ReadModelRepository:
    ...
    def _get_result_from_projection(self, result_id: str, *, allow_stale_projection: bool = False) -> dict[str, Any] | None:
        ...
        if payload is None:
            return None
        if self._should_refresh_result_trace_bundle(trace_id):
            self._refresh_single_trace_if_needed(trace_id)
            refreshed_payload, refreshed_trace_id = self._result_projection_payload(result_id)
            if refreshed_payload is not None:
                payload = refreshed_payload
                trace_id = refreshed_trace_id or trace_id
    ...
    def get_result(self, result_id: str) -> dict[str, Any] | None:
        ...
        Boundary behavior:
            ...
            Query-side refresh may change freshness, but it must not change the
            externally visible response schema.
        """
        self.refresh_if_needed()
    ...
    def get_result_from_projection(self, result_id: str) -> dict[str, Any] | None:
        ...
        if not bool(readiness.get('projectionAvailable')):
            raise ReadModelSyncRequiredError('SQLite read model has no materialized projection rows; explicit repair is required before serving result details')
        return self._get_result_from_projection(result_id, allow_stale_projection=True)
    ...
    def trace_bundle(self, trace_id: str) -> dict[str, Any]:
        self.refresh_if_needed()
        if self._should_refresh_result_trace_bundle(str(trace_id)):
            self._refresh_single_trace_if_needed(str(trace_id))
        with self.connection() as conn:
