    ...
    def on_command(self, msg: String) -> None:
        ...
        if command == 'set_mode':
            ...
            try:
                ...
                changed = False
            if changed and target_mode == SupervisorMode.AUTO.value:
                self._publish_control(RESUME_COMMAND)
            elif changed and target_mode == SupervisorMode.PAUSED.value:
                self._publish_control(PAUSE_COMMAND)
            elif changed and target_mode == SupervisorMode.MAINTENANCE.value:
                self._publish_control(ENTER_MANUAL_COMMAND)
            elif changed and target_mode == SupervisorMode.STOPPED.value:
