...
def test_result_store_detail_reconciles_single_trace_changes_without_global_rebuild(tmp_path: Path) -> None:
    runtime = tmp_path / 'runtime'
    _prepare_runtime_logs(runtime)
...
def test_result_store_detail_refresh_updates_top_level_projection_fields(tmp_path: Path) -> None:
    runtime = tmp_path / 'runtime'
    _prepare_runtime_logs(runtime)
