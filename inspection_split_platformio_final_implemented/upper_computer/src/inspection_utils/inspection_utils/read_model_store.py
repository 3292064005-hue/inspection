...
class ReadModelStore:
    ...
    def update_sync_token_metadata(self, conn: sqlite3.Connection) -> str:
        source_files = self.source_file_tokens(); trace_token = self.next_trace_token(); sync_token = '|'.join([source_files['result_csv'], source_files['summary_jsonl'], source_files['replay_manifest_jsonl'], source_files['artifact_index_jsonl'], trace_token])
        conn.execute("INSERT OR REPLACE INTO metadata(key, value) VALUES ('sync_token', ?)", (sync_token,)); self.write_sync_state(sync_token=sync_token, source_files=source_files, trace_token=trace_token); return sync_token
