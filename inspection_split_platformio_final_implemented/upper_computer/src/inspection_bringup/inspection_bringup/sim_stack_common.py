...
def build_simulated_stack(*, deprecation_notice: str = '') -> LaunchDescription:
    ...
    actions = [
        ...
        DeclareLaunchArgument('recipe_root', default_value=str(bringup_share / 'config' / 'recipes')),
        DeclareLaunchArgument('frontend_dist', default_value=str(gateway_share / 'frontend' / 'dist')),
        DeclareLaunchArgument('users_path', default_value=str(gateway_share / 'config' / 'system' / 'hmi_users.yaml')),
    ]
