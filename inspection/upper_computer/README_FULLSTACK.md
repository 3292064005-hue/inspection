...
- 仓库默认不再内置任何明文账号/口令。
- 若 runtime root 下的 `config/system/hmi_users.yaml` 不存在，网关会在 `logs/runtime/bootstrap/bootstrap_admin.yaml` 生成一次性引导管理员。若显式用户文件已存在但为空且开启严格用户配置，网关会拒绝启动。
...
### 1.1 资源路径与运行目录
- 前端静态资源默认从已安装包 share 目录解析；配方库与用户库的运行态写入路径默认锚定到 runtime root，避免把激活状态、修订历史或 bootstrap 账号写回 share 目录。
...
- `GET /api/v1/audit`
- `WS /ws/v1?ticket=...`
- WebSocket 默认继续广播全部主题；如需收窄订阅面，可使用 `WS /ws/v1?ticket=...&topics=station,system`。
- `GET /api/v1/health` 现在会同时返回 runtime/node/executor/spin thread 就绪状态，以及 actionExecution.transportMode/actionExecutorExpected/transportReady/transportObserved，便于区分 API 存活、本地动作传输已绑定，以及是否已经观察到执行面更新事件。
...
- 网关额外提供 `GET /api/v1/results/read-model/status`，用于显式暴露 read-model projection 的健康与 repair 状态；repair 维护状态会持久化到 `logs/runtime/results/read_model_maintenance_status.json`。
