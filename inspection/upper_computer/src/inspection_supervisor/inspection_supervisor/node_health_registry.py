...
class NodeHealthRegistry:
    ...
    def __post_init__(self) -> None:
        ...
        for name in self.expected_nodes:
            ...
            self.nodes[name] = NodeHealth(name=name, required=name in self.required_nodes, criticality=criticality)

    def ensure_node(self, name: str) -> NodeHealth:
        if name not in self.nodes:
            criticality = self.node_classes.get(name, 'optional')
            self.nodes[name] = NodeHealth(name=name, required=criticality != 'optional', criticality=criticality)
