...
class TelemetryGatewayQueryService:
    ...
    def list_bridges(self) -> list[dict[str, Any]]:
        runtime_snapshot = self.context.log_root / 'telemetry_bridges.json'
        if runtime_snapshot.exists():
            bridges = TelemetryService(str(runtime_snapshot)).list_bridges()
            if bridges:
                return bridges
        return TelemetryService(str(self.context.telemetry_config_path)).list_bridges()
