...
def fetch_result_ids_for_trace_ids(conn: sqlite3.Connection, trace_ids: list[str]) -> list[str]:
    normalized = [str(item) for item in trace_ids if str(item)]
    if not normalized:
        return []
    placeholders = ','.join('?' for _ in normalized)
    rows = conn.execute(
        f'SELECT result_id FROM result_lookup WHERE trace_id IN ({placeholders}) ORDER BY timestamp DESC, result_id DESC',
        tuple(normalized),
    ).fetchall()
