...
def test_top_level_release_validation_script_exists() -> None:
    root = Path(__file__).resolve().parents[3]
    text = (root / 'scripts' / 'run_release_validation.sh').read_text(encoding='utf-8')
    assert 'validate_split_environment.py' in text
    assert 'platformio run -d firmware/stm32_station_platformio' in text
