...
def test_read_model_repository_requires_explicit_repair_after_trace_changes(tmp_path: Path) -> None:
    root = tmp_path / 'runtime'
    results = root / 'results'
    traces = root / 'traces'
    results.mkdir(parents=True)
    ...
    repository = ReadModelRepository(root, policy=ReadModelPolicy(mode='hot', bootstrap_repair_on_empty_db=True, allow_runtime_repair_on_sync_mismatch=False, fallback_legacy_reads=False, query_side_trace_refresh='disabled'))
    ...
    try:
        repository.get_result('TRACE-002')
    except ReadModelSyncRequiredError:
        pass
    else:
        raise AssertionError('stale trace details must require explicit repair')
