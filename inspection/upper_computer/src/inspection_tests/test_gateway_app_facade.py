...
class _RosBridge:
    ...
    def publish_supervisor_command(self, command: str, *, mode: str = '', reason: str = '') -> bool:
        self.supervisor_commands.append({'command': command, 'mode': mode, 'reason': reason})
        return True
...
def test_call_start_consumes_runtime_recipe_and_batch_overrides(tmp_path: Path) -> None:
    app = _build_app(tmp_path)
    app.activate_recipe('recipe-a', operator='tester')
    app.refresh_recipes()
    bridge = _RosBridge(start_result=ServiceCallResult(True, 'started'))
    ...
    ok, message = app.call_start(recipe_id='recipe-b', batch_id='BATCH-OVERRIDE')

    assert ok is True
    assert message == 'started'
    assert bridge.start_calls[0]['recipe_id'] == 'recipe-b'
    assert bridge.start_calls[0]['batch_id'] == 'BATCH-OVERRIDE'
    assert app.state.active_recipe_id == 'recipe-b'
    assert app.state.pending_batch_id == 'BATCH-OVERRIDE'


def test_request_maintenance_mode_publishes_supervisor_command_and_updates_snapshot(tmp_path: Path) -> None:
    app = _build_app(tmp_path)
    bridge = _RosBridge()
    app.bind_ros_bridge(bridge)

    payload = app.request_maintenance_mode(True, actor='tester')

    assert bridge.supervisor_commands == [{'command': 'set_mode', 'mode': 'MAINTENANCE', 'reason': 'gateway_maintenance_request:tester'}]
    assert payload['maintenance']['requested'] is True
    assert payload['maintenance']['enabled'] is False
    assert payload['maintenance']['transitionState'] == 'ENTERING'
    assert payload['supervisorMode'] != 'MAINTENANCE'
