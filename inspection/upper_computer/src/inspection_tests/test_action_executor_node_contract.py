...
def test_gateway_runtime_disables_native_action_servers_when_executor_enabled() -> None:
    ...
    assert 'GatewayRosBridge(' in text
    assert 'executor_enabled=self._executor_enabled' in text
...
def test_action_executor_node_deduplicates_before_contract_lookup() -> None:
    ...
    contract_idx = text.index("contract = ensure_action_submit_allowed(normalized_kind)")
