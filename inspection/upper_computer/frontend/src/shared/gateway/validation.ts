...
function parseStationMaintenanceState(value: unknown): StationMaintenanceState {
  const object = expectObject(value, 'StationMaintenanceState');
  return {
    requested: expectBoolean(object.requested, 'StationMaintenanceState.requested'),
    enabled: expectBoolean(object.enabled, 'StationMaintenanceState.enabled'),
    transitionState: expectString(object.transitionState, 'StationMaintenanceState.transitionState') as StationMaintenanceState['transitionState'],
    supervisorMode: expectString(object.supervisorMode, 'StationMaintenanceState.supervisorMode'),
    source: expectOptional(object.source, 'StationMaintenanceState.source', expectString),
  };
}
...
export function parseStationStateSnapshot(value: unknown): StationStateSnapshot {
  ...
  const maintenance: StationMaintenanceState = isObject(object.maintenance)
    ? parseStationMaintenanceState(object.maintenance)
    : {
        requested: false,
        enabled: false,
        transitionState: 'LOCKED',
        supervisorMode: typeof object.supervisorMode === 'string' ? expectString(object.supervisorMode, 'StationStateSnapshot.supervisorMode') : 'STOPPED',
        source: 'default',
      };
  return {
    ...
    supervisorMode: typeof object.supervisorMode === 'string' ? expectString(object.supervisorMode, 'StationStateSnapshot.supervisorMode') : maintenance.supervisorMode,
    maintenance,
  };
}
...
export function parseCameraFrame(value: unknown): CameraFrame {
  ...
  return {
    ...
    semantic: typeof object.semantic === 'string' ? expectString(object.semantic, 'CameraFrame.semantic') as CameraFrame['semantic'] : undefined,
    sourceEvent: typeof object.sourceEvent === 'string' ? expectString(object.sourceEvent, 'CameraFrame.sourceEvent') : undefined,
    description: typeof object.description === 'string' ? expectString(object.description, 'CameraFrame.description') : undefined,
  };
}
...
export function parseReadModelStatus(value: unknown): ReadModelStatus {
  const object = expectObject(value, 'ReadModelStatus');
  return {
    mode: expectString(object.mode, 'ReadModelStatus.mode'),
    degraded: expectBoolean(object.degraded, 'ReadModelStatus.degraded'),
    lastError: expectString(object.lastError, 'ReadModelStatus.lastError'),
    repairRequired: expectBoolean(object.repairRequired, 'ReadModelStatus.repairRequired'),
    projectionAvailable: expectBoolean(object.projectionAvailable, 'ReadModelStatus.projectionAvailable'),
    fallbackEnabled: expectBoolean(object.fallbackEnabled, 'ReadModelStatus.fallbackEnabled'),
    querySurface: expectString(object.querySurface, 'ReadModelStatus.querySurface'),
    maintenanceState: expectString(object.maintenanceState, 'ReadModelStatus.maintenanceState'),
    repairRunning: expectBoolean(object.repairRunning, 'ReadModelStatus.repairRunning'),
    lastRepairAt: expectString(object.lastRepairAt, 'ReadModelStatus.lastRepairAt'),
    lastRepairReason: expectString(object.lastRepairReason, 'ReadModelStatus.lastRepairReason'),
    sourceSyncToken: expectString(object.sourceSyncToken, 'ReadModelStatus.sourceSyncToken'),
    materializedSyncToken: expectString(object.materializedSyncToken, 'ReadModelStatus.materializedSyncToken'),
  };
}
...
function parseOrchestratorAdviceAction(value: unknown): { action: string; reason?: string } {
  const object = expectObject(value, 'OrchestratorAdviceAction');
  return {
    action: expectString(object.action, 'OrchestratorAdviceAction.action'),
    reason: expectOptional(object.reason, 'OrchestratorAdviceAction.reason', expectString),
  };
}

function parseOrchestratorAdvice(value: unknown) {
  const object = expectObject(value, 'OrchestratorAdviceEvent');
  const traceValue = object.trace;
  return {
    node: expectString(object.node, 'OrchestratorAdviceEvent.node'),
    tree: expectString(object.tree, 'OrchestratorAdviceEvent.tree'),
    status: expectString(object.status, 'OrchestratorAdviceEvent.status'),
    durationMs: expectNumber(object.durationMs, 'OrchestratorAdviceEvent.durationMs'),
    time: expectString(object.time, 'OrchestratorAdviceEvent.time'),
    actions: expectArray(object.actions ?? [], 'OrchestratorAdviceEvent.actions', parseOrchestratorAdviceAction),
    trace: typeof traceValue === 'string' || (typeof traceValue === 'object' && traceValue !== null) ? traceValue as Record<string, unknown> | Record<string, unknown>[] | string : undefined,
  };
}
