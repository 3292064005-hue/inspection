  ...
  <header class="sticky top-0 z-20 border-b border-slate-800/70 bg-slate-950/90 backdrop-blur">
    <div class="mx-auto flex max-w-[1920px] items-center justify-between gap-4 px-4 py-3">
      <div class="flex items-center gap-4">
        ...
        <span v-if="stationStore.snapshot.maintenance.enabled || stationStore.snapshot.maintenance.requested" class="status-pill border-amber-400/25 bg-amber-500/15 text-amber-200">
          {{ stationStore.snapshot.maintenance.enabled ? '维护模式已生效' : '维护模式切换中' }}
        </span>
      </div>
