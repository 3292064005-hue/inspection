...
<template>
  <div class="flex h-full flex-col gap-4">
    ...
    <div v-if="hasReadModelIssue || readModelStatus" class="panel p-4">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div class="panel-title">结果读模型状态</div>
          <div class="mt-2 text-sm" :class="hasReadModelIssue ? 'text-amber-200' : 'text-emerald-200'">{{ readModelStatusLabel }}</div>
          <div class="mt-2 text-xs text-slate-400">
            mode={{ readModelStatus?.mode ?? '--' }} · projection={{ readModelStatus?.projectionAvailable ? 'ready' : 'missing' }} · querySurface={{ readModelStatus?.querySurface ?? '--' }}
          </div>
          <div v-if="readModelStatus?.lastError" class="mt-2 text-sm text-rose-200">{{ readModelStatus.lastError }}</div>
          <div v-if="readModelStatus?.lastRepairAt" class="mt-2 text-xs text-slate-500">上次 repair：{{ formatDateTime(readModelStatus.lastRepairAt) }}</div>
        </div>
        <div class="flex flex-wrap gap-3">
          <button class="rounded-2xl border border-slate-700 px-4 py-3 text-sm font-semibold text-slate-100 hover:border-slate-500" @click="refreshReadModelStatus(true)">刷新状态</button>
          <button
            v-if="canRepairReadModel"
            :disabled="repairingReadModel || !!readModelStatus?.repairRunning"
            class="rounded-2xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm font-semibold text-amber-100 disabled:cursor-not-allowed disabled:opacity-50"
            @click="repairReadModel"
          >
            {{ repairingReadModel || readModelStatus?.repairRunning ? 'Repair 中…' : '执行 repair' }}
          </button>
        </div>
      </div>
    </div>
      ...
      <div class="panel min-h-0 p-4">
        ...
        <div class="mt-4 grid gap-3">
          <button
            ...
            :class="selectedId === item.id ? 'border-sky-400/40 bg-sky-500/10' : 'border-slate-800/70 bg-slate-950/60 hover:border-slate-600'"
      ...
      <div class="flex min-h-0 flex-col gap-4">
        ...
        <div class="panel p-4">
          ...
          <div class="mt-4 space-y-2 text-sm text-slate-300">
            <div v-for="entry in batchSummary" :key="entry[0]" class="rounded-2xl border border-slate-800/70 bg-slate-950/60 px-4 py-3">
              <div class="font-semibold text-slate-100">{{ entry[0] }}</div>
              <div class="mt-1">总数 {{ entry[1].count }} · OK {{ entry[1].ok }} · NG {{ entry[1].ng }}</div>
            </div>
          </div>
        </div>
      </div>
