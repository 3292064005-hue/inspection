...
mkdir -p "$OUT_DIR"
ARCHIVE_PATH="${OUT_DIR}/inspection_split_release_$(date +%Y%m%d_%H%M%S).tar.gz"
...
tar \
  ...
  -czf "$ARCHIVE_PATH" \
  -C "$ROOT_DIR" \
  README.md IMPLEMENTATION_SUMMARY.md docs firmware upper_computer release scripts .github
