...
## 查询侧 repair 约定
- 结果与回放详情默认只读取 materialized projection。
- 当 read model 过期时，接口返回 503 + `readModelStatus`，不再在请求线程内做单 trace 刷新。
- 网关显式 repair 接口：`POST /api/v1/results/read-model/repair`（维护权限）

## 协议版本约定
- 当前 split delivery 统一使用 canonical `v1` 协议标签。
- release manifest、STM32 capability payload、上位机兼容矩阵都以 `v1` 作为机器判定真值。
